...
function init(){
    ...
    savedGraph.edges.forEach(element => {
        let edge = SavedAddArrow(temphash[element.x], temphash[element.y], layer);
        globaljs.graph.addEdge(edge); 
    });
    
    
    console.log(globaljs.nodes);
    for (let i in globaljs.nodes){
        console.log(globaljs.nodes[i]);
...
function addArrow(node1, node2, layertoadd) {
    ...
    if (node2 == null) {
        ...
        arrow = new Konva.Line({
            ...
            shape1.getX() + (shape1.width() / 2)+-radius * Math.cos(angle),
            shape1.getY()+ (shape1.height()/2) + radius * Math.sin(angle)],
            pointerLength: 6,
            pointerWidth: 4,
            fill: 'black',
            stroke: 'black',
    ...
    console.log(globaljs.graph.numberOfEdges)
    edge = new tfEdge(node1, node2, arrow, globaljs.graph.numberOfEdges)
    //console.log(graph)
    globaljs.edges[globaljs.graph.numberOfEdges]=arrow;
